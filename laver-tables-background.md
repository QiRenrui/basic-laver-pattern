...
I believe the broader picture may be considerably stronger than the theorem currently proves. LTY appears to be an exceptionally strong explicit recursive ordinal notation system, and quantities associated with the Laver-table function---for example values often denoted by expressions such as $q(5)$ in the googology literature---appear to be vastly larger than Graham's number or $\operatorname{TREE}(3)$. Claims at this scale should be understood carefully: some are rigorous lower bounds, while others depend on ongoing ordinal analysis and should presently be regarded as conjectural or heuristic.
